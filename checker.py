...
# Dedicated whitelist (BS) AmneziaWG from bs/*.conf — published as pattng-bs.json.
...
def load_whitelist_bs(token=""):
    """Classic 3 Cloudflare AWG (working PattNG JSON) plus ZIP configs."""
    items = []
    seen = set()
    try:
...
def happ_subscription_text(uris):
    ...
    lines = [
        ...
        if not ln.startswith("#profile-title:") and not ln.startswith("#profile-update-interval:")
    ]
    payload = "\n".join(lines)
    return (
...
def main():
    ...
    # Dedicated happ-bs / pattng-bs already written from bs/*.conf + classic pins.
